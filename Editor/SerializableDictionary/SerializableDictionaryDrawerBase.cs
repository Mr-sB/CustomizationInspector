...
namespace CustomizationInspector.Editor
{
    public abstract class SerializableDictionaryDrawerBase : PropertyDrawer
    {
        ...
        public SerializableDictionaryDrawerBase(bool showAdd)
        {
            mShowAdd = showAdd;
        }
        ...
        {
            if (mShowAdd && mToAddContent == null)
            ...
            if (EditorGUI.PropertyField(labelRect, property, label, false))
            {
                ...
                if (mShowAdd)
                {
                    ...
                    if (isAddProperty.boolValue)
                    {
                        ...
                        //显示添加的GUI
                        var itemElementNameRect = new Rect(labelRect.x, position.y + propertyHeight, elementNameWidth,
                            EditorGUIUtility.singleLineHeight);
                        //计算属性高度
                        ...
                        var keyRect = new Rect(keyLabelRect.x, position.y + propertyHeight, keyLabelRect.width,
                            keyPropertyHeight);
                        ...
                        var valueRect = new Rect(valueLabelRect.x, position.y + propertyHeight, valueLabelRect.width,
                            valuePropertyHeight);
                        ...
                        var addRect = EditorGUI.IndentedRect(new Rect(labelRect.x, position.y + propertyHeight,
                            lineWidth, EditorGUIUtility.singleLineHeight));
                        propertyHeight += EditorGUIUtility.singleLineHeight;
                        if (GUI.Button(addRect, "Add"))
                        {
                            ...
                            {
                                FieldInfo toAddValueFieldInfo =
                                    type.GetFieldInfoIncludeBase("mToAddValue", mBindingFlags);
